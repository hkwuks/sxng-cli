...
export type JsonInputResult =
    ...
    | { ok: false; code: 'MISSING_JSON_INPUT' | 'MULTIPLE_JSON_INPUTS' | 'JSON_FILE_READ_FAILED' | 'UNSUPPORTED_JSON_ENCODING' | 'INVALID_JSON'; message: string };
...
/** Read exactly one inline JSON value or UTF-8 JSON file. */
export function readSingleJsonInput(sources: JsonInputSource[]): JsonInputResult {
    ...
    if (!source.file) return parseJson(source.value!, source.option);

    let raw: Buffer;
    try {
        raw = readFileSync(source.value!);
    } catch {
